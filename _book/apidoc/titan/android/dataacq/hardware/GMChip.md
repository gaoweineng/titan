...
国密芯片数据采集. 通过DataAcqService.getComponent(GMChip.class)获取实例。
...
<tr>
  <td width="100">ENCRYPT_IC_MODEL</td>
  <td>String</td>
  <td>获取国密芯片型号</td>
</tr>
</table>
...
不支持应用直接实例化，而是通过DataAcqService.getComponent(GMChip.class)接口获取实例。
...
TitanContext titan = TitanContext.getTitanContext(context);
 try {
 	...
 	GMChip chip = dataAcq.getComponent(GMChip.class);
 	//获取国密芯片型号
    String model = chip.getUsage(GMChip.ENCRYPT_IC_MODEL);
