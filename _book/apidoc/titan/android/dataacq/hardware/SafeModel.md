...
安全模块数据采集. 通过DataAcqService.getComponent(SafeModel.class)获取实例。
...
不支持应用直接实例化，而是通过DataAcqService.getComponent(SafeModel.class)接口获取实例。
...
TitanContext titan = TitanContext.getTitanContext(context);
 try {
 	...
 	SafeModel safeModel = dataAcq.getComponent(SafeModel.class);
	//获取安全模块型号
 	String model = safeModel.getUsage(SafeModel.SAFE_MODEL);
